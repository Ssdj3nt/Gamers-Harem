...
def register():
    if request.method == 'POST':
        ...
        existing_user_nickname = mongo.db.users.find_one({'nickname': nickname})

        if existing_user_email or existing_user_nickname:
            error_message = "Email o nickname già registrati."
            return render_template('index.html', registration_error=True, error_message=error_message,initial_login_state=True)
